#!/usr/bin/env python3
"""
Handler for no-grid submissions.
"""

import json
from pathlib import Path

def validate_no_grid(data):
    """
    Validate no-grid submission.
...
def run(parsed_data, issue_data):
    ...
    """
    print("processing no-grid submission...")

    #Validate
    errors = validate_no_grid(parsed_data)
    if errors:
    ...
    #Create JSON
    entry = create_no_grid_json(parsed_data)

    #Write file
    output_dir = Path("no-grid")
