#!/usr/bin/env python3
"""
Handler for forcing-dataset submissions.
"""

import json
from pathlib import Path

def validate_forcing_dataset(data):
    """
    Validate forcing-dataset submission.
...
def create_forcing_dataset_json(data):
    """
...
def run(parsed_data, issue_data):
    ...
    """
    print("processing forcing-dataset submission...")

    #Validate
    errors = validate_forcing_dataset(parsed_data)
    if errors:
    ...
    #Create JSON
    entry = create_forcing_dataset_json(parsed_data)

    #Write file
    output_dir = Path("forcing-dataset")
